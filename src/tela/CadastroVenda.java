...
import utilitario.JtextFieldSomenteNumeros;
...
/**  
 * 
 * <p>A classe CadastroCliente cria um "JDialog" (janela) que cadastra as vendas.</p>  
...
public class CadastroVenda extends JDialog {
    ...
    private boolean verificaErroEditarProduto = false;
    
    public boolean getVerificar() {
		return verificaErroEditarProduto;
	}
	...
	/**
	 * Método que exlcui todas as linhas da tabela de itens.
	 * 
	 */
	public static void limpaTabelaItem() {
    ...
    /**
	 * Método que adiciona itens na lista da venda.
	 * 
	 */
    private void botaoAdicionar(ActionEvent evt) {
    	try {
    ...
    /**
	 * Método que remove itens na lista da venda.
	 * 
	 */
    private void botaoRemover(ActionEvent evt) {
    ...
    /**
	 * Método que trata erro causado pelo usuario em caso de fechmamento da janela com itens adicionados.
	 * 
	 */
    		...
    		for(Venda v : ControleVenda.bancoDeDadosVenda) {
    			if(v.getId() == id) {
    				if(v.getItens().size() == 0) {
    					verificaErroEditarProduto = true;
    					dispose();
    					break;
    				}
    				else {
    					TelaPrincipal.limpaTabelaVenda();
    	            	TelaPrincipal.atualizarTabelaVenda();
    					dispose();
    				}
    			}
    		}
    ...
    /**
	 * Método que filtra qual vendedor participa da venda.
	 * 
	 */
    ...
    /**
	 * Método que filtra qual cliente participa da venda.
	 * 
	 */
    ...
    private void botaoSalvar(ActionEvent evt) {
		...
		if (id != null) {
			for(Venda v : ControleVenda.bancoDeDadosVenda){
				if(id == v.getId() && v.getItens().size() > 0) {
					...
					dispose();
					break;
				} else if(v.getItens().size() == 0) {
					JOptionPane.showMessageDialog(null,
							"Deve haver pelo menos 1 item na venda, se deseja remover tudo opte por excluir.");
					break;
				}
			}
		}
    		...
    		if(itens.size() == 0) {
    			JOptionPane.showMessageDialog(null,"Adicione itens para reazizar a venda!");
    		}
    		else {
            	...
            	Object[] cadastroVenda = new Object[]{id,vendedor,cliente,data,df.format(valor),situacao,pagamento};
